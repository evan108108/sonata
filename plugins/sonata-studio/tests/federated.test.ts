...
const SCOUT_SONATA = process.env["STUDIO_SCOUT_SONATA"] ?? "http://192.168.0.17:3211";
...
async function runOnce(attempt: number): Promise<Fixture> {
  ...
  try {
    ...
    try {
      const ping = await fetch(`${SCOUT_SONATA}/api/system/ping`);
      if (!ping.ok) {
        throw new Error(`HTTP ${ping.status}`);
      }
    } catch (e) {
      throw new Error(
        `Scout Sonata /api/system/ping unreachable at ${SCOUT_SONATA} — ${
          e instanceof Error ? e.message : String(e)
        }`,
      );
    }
