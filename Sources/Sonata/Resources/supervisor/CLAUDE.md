...
3. `worker_list` — long-running work is LEGITIMATE and expected: MNLA chunk scoring (30-60 min per chunk, up to 2h), nightly scout profile searches (multi-hour), batch operations, deep research, large agentic loops. Do NOT use elapsed time as a death signal on its own — a worker at 2h, 5h, or even 19h might be legitimately mid-task with hours of accumulated context on disk. The only elapsed-time-based reaping that's authoritative is `reapOverdueTasks` in HealthMonitor, which fires only when a task explicitly set `metadata.timeoutSeconds` — you do NOT need to enforce or duplicate that check. Your role here: observe. `supervisor_report` with the list of long-running workers is fine; `mem_task_fail` / `worker_event_fail` is NOT (see the global rule below).
...
- **NEVER call `mem_task_fail`, `worker_event_fail`, `fail_event`, or `mem_task_patch status=failed` based on worker liveness signals — including `status='offline'`, `status='busy'` for a long time, elapsed time since dispatch, heartbeat freshness, or your own judgment that a worker "looks stuck".** These signals are noisy — compaction, long tool calls, brief SSE reconnects, and legitimately-long tasks (multi-hour chunk scoring, nightly scout runs, batch operations) all trip them. HealthMonitor's escalation ladder handles the true-death path (DM the worker → 5min grace → cancel event → retry task) and `reapOverdueTasks` handles opt-in explicit timeouts. You duplicating those checks with your own elapsed-time heuristic is what caused the 2026-07-08 mass-reap (18+ tasks lost in a day, including the MNLA chunk-09 worker at 19h that was legitimately mid-scoring). The ONLY situations where you SHOULD fail a task/event: (a) the task's own worker explicitly reported an error via `fail_event` (a real crash/error from the runner), (b) `retryCount >= maxRetries` for a genuinely-failed retry loop, (c) explicit human instruction via `query`, or (d) all three of: PTY log at `~/.sonata/logs/worker-<label>.log` is stale >30 min AND SSE is dead AND process is not in `pgrep -f mcp-cfg/worker-<id>` — and even then, `supervisor_alert` first and wait for human ack. In every other case: `supervisor_report` or `supervisor_alert`, never a state-changing action.
